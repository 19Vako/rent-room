...
export default function ActionModal({ isOpen, type, message, onClose }: {
  isOpen: boolean;
  type: "success" | "error";
  message: string;
  onClose: () => void;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-lg shadow-xl p-6 w-[90%] max-w-sm animate-in fade-in zoom-in duration-200">
        <div className={`flex items-center justify-center w-12 h-12 mx-auto rounded-full mb-4 ${type === 'success' ? 'bg-green-100' : 'bg-red-100'}`}>
          {type === 'success' ? (
