...
export default function DashboardCalendar() {
  ...
  const { selectedRoom, startDate, endDate, setStartDate, setEndDate } = useSettingData();
        ...
        <div className="min-w-[800px] border-t border-l border-gray-200 relative">
          ...
          {isLoading && (
            <div className="absolute inset-0 bg-white/70 z-20 flex items-center justify-center">
              <span className="text-black font-medium text-lg bg-white px-4 py-2 rounded shadow">Loading events...</span>
            </div>
          )}

          <div className="grid grid-cols-7 bg-white">
            {DAYS_OF_WEEK.map((day) => (
              <div key={day} className="py-2 px-3 border-r border-b border-gray-200 text-sm text-gray-700 font-medium">
                {day}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 bg-[#f7f7f7]">
            {blanks.map((blank) => (
              <div key={`blank-${blank}`} className="h-32 border-r border-b border-gray-200 bg-gray-50"></div>
            ))}

            {days.map((day) => {
              const currentDateStr = formatDate(YEAR, MONTH, day);
             
              const isSelected = currentDateStr >= startDate && currentDateStr <= endDate;
