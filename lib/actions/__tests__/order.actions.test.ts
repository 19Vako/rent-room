 ...
 */
import { createOrder, cancelOrder, getUserOrders, getAvailableRooms } from '@/lib/actions/order.actions';
...
describe('Server Actions: Orders', () => {
  ...
  describe('createOrder', () => {
    it('it must return an error if the user is not logged in', async () => {
      ...
      const result = await createOrder('room_id', '2026-05-01', '2026-05-05', 100, 2);
      
      expect(result).toEqual({ success: false, error: 'Please log in to the system' });
    });
    ...
    it('it must successfully create an order and update the cache', async () => {
      ...
      const result = await createOrder(mockRoomId, '2026-05-01', '2026-05-05', 100, 2);
  ...
  describe('getAvailableRooms', () => {
    ...
    it('it must return an error if the check-out date is earlier than the check-in date', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('The check-out date must be later than the check-in date.');
    });
  ...
  describe('getUserOrders', () => {
    ...
    it('it must return orders for regular users', async () => {
      const mockUserId = '507f1f77bcf86cd799439011';
      (auth as jest.Mock).mockResolvedValue({ user: { id: mockUserId, role: 'USER' } });
      ...
      expect(mockFind).toHaveBeenCalledWith({ userId: new ObjectId(mockUserId) });
    });

    it('it must return all future orders for ADMIN', async () => {
      const mockAdminId = '507f1f77bcf86cd799439099';
      (auth as jest.Mock).mockResolvedValue({ user: { id: mockAdminId, role: 'ADMIN' } });
