 ...
 */
import { getAllRooms, getRoomById, blockRoomDates, unblockRoomDates, createRoom, deleteRoom, updateRoom, getRoomCalendarEvents, addRoomImage, removeRoomImage } from '@/lib/actions/room.actions';
...
describe('Room Actions', () => {
  ...
  it('should return error if end date is before start date', async () => {
    const result = await blockRoomDates(roomId, '2026-05-10', '2026-05-01', reason);
  ...
  it('should return error if there is an active booking', async () => {
    mockFindOne.mockResolvedValueOnce(null);
    mockFindOne.mockResolvedValueOnce({ _id: new ObjectId(), status: 'CONFIRMED' });

    const result = await blockRoomDates(roomId, startDate, endDate, reason);

    expect(result.success).toBe(false);
    expect(result.error).toBe('Cannot block dates: there is an active booking.');
  });
  ...
  it('should throw an error if the user is not an admin', async () => {
    ...
    await expect(unblockRoomDates(roomId, startDate, endDate))
  ...
  describe('deleteRoom', () => {
    ...
    it('should throw an error if the user is not an admin', async () => {
      ...
      await expect(deleteRoom(roomId))
        .rejects.toThrow("Only admins can delete rooms");
    });
  ...
  describe('updateRoom', () => {
    ...
    it('should throw an error if the user is not an admin', async () => {
      ...
      await expect(updateRoom(roomId, updateData))
        .rejects.toThrow("Only admins can update rooms");
    });
  ...
  describe('getRoomCalendarEvents', () => {
    ...
    describe('removeRoomImage', () => {
      ...
      it('should throw error if user is not admin', async () => {
        ...
        await expect(removeRoomImage(roomId, mockImageUrl))
          .rejects.toThrow("Only admins can remove images from rooms");
      });
    });
