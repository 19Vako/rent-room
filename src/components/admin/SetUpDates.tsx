...
import { useSettingData, getTodayFormatted } from "@/src/store/useSettingData";
...
export default function SetUpDates() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (roomStatus !== "AVAILABLE") {
        ...
        if (!blockResult.success) {
          setModal({
            isOpen: true,
            type: "error",
            message: blockResult.error || "Error",
          });
          setIsSaving(false);
          return;
        }
      } else {
        const unblockResult = await unblockRoomDates(
          selectedRoom.id.toString(),
          startDate,
          endDate,
        );
        if (!unblockResult.success) {
          setModal({
            isOpen: true,
            type: "error",
            message: unblockResult.error || "Error",
          });
          setIsSaving(false);
          return;
        }
      ...
      setModal({
        isOpen: true,
        type: "success",
        message: "Dates and settings updated successfully!",
      });
    } catch (error) {
      console.error(error);
      setModal({
  ...
  if (!selectedRoom)
    return <div className="p-5 text-black">Loading room data...</div>;
