...
import { useSearchRoomsStore } from "@/src/store/useSearchRoomsStore";
import { calculateOrderDetails } from "@/src/lib/utils/calculateOrder";
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      if (response.success) {
        setMessage({
          type: "success",
          text: "The room has been successfully booked.",
        });
      } else {
        setMessage({
          type: "error",
          text: response.error || "Failed to book the room.",
        });
      }
    } catch (error) {
    ...
    <div className="w-full lg:w-[35%] flex flex-col gap-6 flex-shrink-0">
      <div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <span
            className={`px-4 py-2.5 rounded-sm text-sm font-semibold border ${
      ...
      <button
        ...
        disabled={
          !nights || nights <= 0 || isLoading || message?.type === "success"
        }
